<template>
	<transition name="fade">
		<div v-if="isRegLogin" class="bgShadow flex jc-c ai-c">
			<div class="fixedCard bg-white pb-4" :class="{isReg:isRegLogin===2}">
				<div class="logo flex fd-c jc-e ai-c pt-5">
					<div class="closeBtn text-closeBtn btn" @click="closeCard">×</div>
				...
				<div class="cardFooter">
					<p v-if="isRegLogin===2" class="mt-4">
						<span>已有帐号？</span>
						<span class="text-blue btn" @click="toLogin">登录</span>
					</p>
					<p v-else class="mt-4 flex px-4">
						<span class="flex-1 text-blue btn" style="text-align: left">忘记密码？</span>
						<span>新用户？</span>
						<span class="text-blue btn" @click="toReg">注册</span>
					</p>
					<button class="regBtn bg-blue text-white mt-4 btn">{{isRegLogin===2?'快速注册':'快速登录'}}</button>
				</div>
			</div>
		</div>
	</transition>
